...
/**
 * EltakoLumSensor. FAH60+FAH63+FIH63
 * Luminosity Sensor parser for Eltako product.
 * EEP id is an old version deprecated in EEP 2.5 specs
 * 
 * spec from Eltako_wireless_system_chapterT_high_res.pdf
 * 
 * FAH60+FAH63+FIH63 (EEP: 07-06-01 plus Data_byte3)
 * ORG = 0x07
 * Data_byte3 = brightness 0 – 100 lux, linear n = 0x00 – 0xFF
 * (only valid if DB2 = 0x00)
 * Data_byte2 = brightness 300 – 30.000 lux, linear n = 0x00 – 0xFF
 * Data_byte1 = -
 * Data_byte0 = DB0_Bit3 = LRN Button
 * (0 = teach-in telegram, 1 = data telegram)
 * for data telegram: 0x0F, for teach-in telegram: 0x87
 * Teach-in telegram BD3..DB0: 0x18, 0x08, 0x0D, 0x87
 * 
 * 
...
public class EltakoLumSensor implements EEPParser {
     ...
     */
    public EltakoLumSensor(float scaleMin, float scaleMax,
	    float smallerScaleMin, float smallerScaleMax) {
    ...
    /**
     * Parses DB2 for luminosity with scale min and max
     * If DB2 parsing returns 0 parses DB3 with smaller scale min and max
     * 
     */
    @Override
	...
	if (packet instanceof RadioPacket4BS) {
		...
		this.currentValue = this.calculateValue(
			radioPacket4BS.getDb3(), this.smallerScaleMin,
			this.smallerScaleMax, BYTE_RANGE_MIN, BYTE_RANGE_MAX);
